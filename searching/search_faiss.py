...
def search_faiss(args, k=1000):
    ...
    queries = pd.read_csv(f"{args.data_dir}/queries/{args.collection}/queries.tsv", **query_reader_params)

    # read qrels
    qrels_reader_params = {'sep': " ", 'names': ["query_id", "doc_id", "relevance"], "usecols": [0,2,3],
                            'header': None, "dtype": {"query_id": str, "doc_id": str}}
    qrels = pd.read_csv(f"{args.data_dir}/qrels/{args.collection}/qrels.txt", **qrels_reader_params)

    # keep only queries with relevant docs
    queries = queries.loc[queries.query_id.isin(qrels.query_id)]

    
    memmapsdir = f"{args.data_dir}/memmaps/{args.model_name}/{args.collection}"
    qrys_encoder = MemmapQueriesEncoding(f"{memmapsdir}/queries.dat",
                                        f"{memmapsdir}/queries_mapping.tsv")

    # load faiss index
    faiss_path = f"{args.data_dir}/vectordb/{args.model_name}/corpora/{corpus}/"
...
if __name__ == "__main__":
    ...
    out = search_faiss(args, k=50000)
    out.to_csv(f"{args.data_dir}/runs/{args.collection}/{args.model_name}_50k.tsv", header=None, index=None, sep="\t")