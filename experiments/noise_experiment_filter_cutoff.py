...
def main(args, index, hyperparams, comb):
    ...
    run = pd.read_csv(f"{args.datadir}/runs/{args.collection}/{args.model_name}_10k.tsv", **run_reader_parmas)
    filtering = dimension_filters.NoiseFilterAll(qrels=qrels, docs_encoder=docs_encoder, qrys_encoder=qrys_encoder,
    ...
    result.to_csv(f"{args.datadir}/performance/performance_cutoff_giga/{args.collection}_{args.model_name}_{filter_name}_{measure_name}_{comb}.csv", index=False)
...
if __name__ == "__main__":
    ...
    import pickle
    hyperparams_filename = f"data/performance/performance_cutoff_giga/hyperparams.pickle"
