...
def main(args, index, hyperparams, comb, loop_id):
    ...
    filtering = dimension_filters.NoiseFilterAll_R(qrels=qrels, docs_encoder=docs_encoder, qrys_encoder=qrys_encoder,
    ...
    result.to_csv(f"{args.datadir}/performance/performance_RM_2/{args.collection}_{args.model_name}_{filter_name}_{measure_name}_{comb}_{loop_id}.csv", index=False)
...
if __name__ == "__main__":
    ...
    import pickle
    #hyperparams_filename = f"data/performance/performance_RM_1/hyperparams_random.pickle"
    #with open(hyperparams_filename, 'rb') as handle:
    #    combinations_dicts = pickle.load(handle)
    
    #hyperparams_filename = f"data/performance/performance_RM_1/hyperparams_{args.collection}_{args.model_name}.pickle"
    hyperparams_filename = f"data/performance/performance_RM_2/hyperparams_{args.collection}_{args.model_name}.pickle"
    with open(hyperparams_filename, 'rb') as handle:
        combinations_dicts = pickle.load(handle)
    ...
    for i, hyperparams in tqdm(enumerate(combinations_dicts), total=len(combinations_dicts)):
        ## repeat the configuration 5 times (since random)
        print('hyperparms: ', hyperparams)
        for j in range(5):
            main(args, index, hyperparams, i, j)